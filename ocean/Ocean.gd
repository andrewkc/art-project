...
func _ready():
	for i in range(-10, 10):
		for j in range(-10, 10):
			...
			water.set_surface_override_material(0, water_resource)
	

# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	time += delta
	if water_resource: water_resource.set_shader_parameter("time", time) 
...
func set_wave_a(a):
	wave_a = a
	if water_resource: water_resource.set_shader_parameter("wave_a", a)

func set_wave_a_dir(a):
	wave_a_dir = a
	if water_resource: water_resource.set_shader_parameter("wave_a_dir", a)

func set_wave_b(b):
	wave_b = b
	if water_resource: water_resource.set_shader_parameter("wave_b", b)

func set_wave_b_dir(b):
	wave_b_dir = b
	if water_resource: water_resource.set_shader_parameter("wave_b_dir", b)

func set_wave_c(c):
	wave_c = c
	if water_resource: water_resource.set_shader_parameter("wave_c", c)

func set_wave_c_dir(c):
	wave_c_dir = c
	if water_resource: water_resource.set_shader_parameter("wave_c_dir", c)
