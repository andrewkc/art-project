...
@export var amplitude = 10: set = set_amplitude
@export var radius = 100: set = set_radius

# Called when the node enters the scene tree for the first time.
func _ready():
	refresh_mesh()
	if not Engine.is_editor_hint(): mesh_instance.create_trimesh_collision()

func refresh_mesh():
	if not Engine.is_editor_hint() and not mesh_instance:
		return
...
func set_noise(n):
	noise = n
	if Engine.is_editor_hint(): refresh_mesh()
